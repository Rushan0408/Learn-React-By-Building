...
export default function Accordian() {
    ...
    function handleSubmit(id: number) {
        if (multi) {
            if (array.includes(id)) setArray(array.filter((temp) => temp != id));
            else setArray([...array,id]);
        }
        else {
            if (array.includes(id)) setArray([]);
            else setArray([id]);
        }
    }
    ...
    return (
        <div className="wrapper flex flex-col items-center justify-center text-2xl text-white min-h-screen ">
            ...
            { (data && data.length > 0 ) ? data.map((item) => (<div key={item.id} className="flex flex-col items-center  pt-4 pl-6 pr-6 pb-2 rounded-3xl mb-4">
                <div className="text-red-400" > {item.question} </div>
                <button type="button" className="cursor-pointer text-xs text-gray-100 px-2 py-1 bg-gray-800 rounded-sm hover:bg-blue-500 focus:outline-none transition"
                    onClick={() => handleSubmit(item.id)}>+</button>
                {array.includes(item.id) && <div className="text-green-400">  {item.answer} </div>}
            </div>
            )) : <div> No data Found ! </div>}
            <button type="button" className="text-2xl cursor-pointer px-4 py-2 bg-gray-800 text-gray-100 rounded-md border border-gray-700 hover:bg-gray-700 transition"
